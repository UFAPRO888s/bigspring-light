...
//   return await axios.get(encodeURI("https://api-ssl.bitly.com/v3/shorten?access_token=23c47f073826eb0cdc1c2abedbb006976e0e0549&longUrl=" + ufaxlink)).then(token => { return token.data.data.url } )
//     //console.log(ssGlink.data.data.url)
  ...
  try {
    ...
    Items.forEach(element => {
      ...
      var fullDATAMD = json2md([{
        ...
        // },
        {
          p: '<a href="' + goo.randomEM() + IMsitemap.randomEM() + '">' + word.randomEM() + "แทงบอลออนไลน์ " + element.title + '</a>'
        }
      ]);
  ...
  try {
    ...
    ItR1.forEach(res2 => {
      ...
      const RESMD1 = {
        ...
        description: wordbl + ' ' + res2.description + '<a href="' + IMsitemap.randomEM() + '">' + wordbl + '</a>' || ':',
    ...
    for (let i = 0; i < SItR1.length; i++) {
      for (let x = 0; x < SItR2.length; x++) {
        ...
        //console.log([mdTEXT]);
        fs.writeFile(`./content/posts/sport/${SItR1[i].urlmake}.md`, HEADER + mdTEXT, (err) => {
          if (err) {
            console.error(err)
            return
          }
